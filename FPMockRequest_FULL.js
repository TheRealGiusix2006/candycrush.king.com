...
function getUserUniverse() {
    ...
    if (!data) {
        // Generate reality episodes (1 through 189)
        const realityEpisodes = [];
        for (let episodeId = 1; episodeId <= 189; episodeId++) {
            // Episodes 1 and 2 have 10 levels; episodes 3–189 have 15 levels.
            const levelCount = (episodeId <= 2) ? 10 : 15;
            const levels = [];
            ...
            realityEpisodes.push({
                id: episodeId,
                levels: levels
            });
        }

        // Generate dreamworld episodes (1201 through 1245)
        const dreamworldEpisodes = [];
        for (let episodeId = 1201; episodeId <= 1245; episodeId++) {
            const levelCount = (episodeId <= 1202) ? 10 : 15;
            const levels = [];
            for (let levelId = 1; levelId <= levelCount; levelId++) {
                levels.push({
                    id: levelId,
            ...
            dreamworldEpisodes.push({
                id: episodeId,
                levels: levels
            });
        }

        // Combine both reality and dreamworld episodes
        const allEpisodes = realityEpisodes.concat(dreamworldEpisodes);
...
// Greek, Japanese, Koreon, Chinese, and Arabic cause issues with the game and thus have been commented out.
    ...
    if (languageKey !== 'en' && supportedLanguages.includes(languageKey)) {
        ...
        resources = window[`gameInitStrings${language.toUpperCase()}`];
        ...
        userProfiles: [
            {
                ...
                "fullName": "Mystic",
                "name": "Mystic",
                "pic": "http://localhost/candycrush/profilepic100x100.png",
                "picSquare": "http://localhost/candycrush/profilepic50x50.png",
                "countryCode": "CA",
                "topEpisode": 189,
                "topLevel": 2825,
                "totalStars": 8475,
                "lastLevelCompletedAt": 0,
                "lastLevelCompletedEpisodeId": 189,
                "lastLevelCompletedLevelId": 2825,
                "friendType": "NONE"
            }
        ],
        userUniverse,
